...
| **Search — sync lifecycle: `businesses`→`sokoni_shops`** | ✅ **Verified** | `sellers` transformer. Each transition observed after a full drain interval and **before** cleanup: create INDEXED, update propagated, delete removed. `create=PASS update=PASS delete=PASS` | 2026-07-24 | Search | — |
| **Search — sync lifecycle: `mechanics`→`sokoni_services`** | ✅ **Verified** | `services` transformer — a **different** transformer, index and schema, so not inferred from the row above. `create=PASS update=PASS delete=PASS` (09:10:04→09:26:55 UTC) | 2026-07-24 | Search | — |
| **Search — indexing latency** | ✅ **Measured** | **~331s across six independent transitions** in two probes — not "the scheduler is configured for 5 minutes" but observed end-to-end propagation. Matches `processAlgoliaQueue` `every 5 minutes` (`algolia-queue.js:158`) | 2026-07-24 | Search | — |
| **Search — backfill/trigger schema parity** | ❌ **Failed** | Backfill produces a **6-field** record where the trigger produces **28** (observed in `sokoni_services`). Recovered documents are present but under-searchable; root cause of the `"clinic"` zero-result. Scoped architectural follow-up with a falsifiable acceptance test — see [[KNOWN_LIMITATIONS]] | 2026-07-24 | Search | Unify onto one transformer definition |
...
| **Variants — schema/indexer parity** | ✅ **Verified** | `npm run check:variants` 33/33. Asserts the browser schema's `ALL_ATTR_KEYS` equals Cloud Functions' `VARIANT_FIELDS`, every `CATEGORY_ATTRS` entry is indexable, and every variant field is both searchable and facetable in `algolia-admin.js` | 2026-07-24 | Search | — |
| **Variants — display (product page + cards)** | ✅ **Verified** | Live browser (Playwright, 430px, production build): fashion → Colour/Size/Material, electronics → Colour/Storage, skincare → Volume, meat → Pack size; **0 empty headings, 0 page errors**; card summaries `Black • M` / `Black • 256GB` / `500ml`; 44px chip touch target | 2026-07-24 | UX | — |
| **Variants — backward compatibility** | ✅ **Verified** | Products with `missing / null / [] / scalar / non-string / whitespace` attributes render no group and no card line; a pre-variant product falls back to the legacy category chips. Covered in both `check:variants` and the browser run | 2026-07-24 | QA | — |
| **Variants — output encoding** | ✅ **Verified** | A stored colour of `"><img src=x onerror=…>` renders as text on **both** the product page and the card path; `window.__XSS` never set | 2026-07-24 | Security | — |
| **Variants — filters (category page)** | ✅ **Verified** | Facets derived from products in view: Colours/Sizes shown for Fashion, **none** for tyres (bar hidden). OR within an attribute, AND across (Black → 2, +XL → 1). Filtered-empty state says "No matches for these filters" with working recovery, not "nothing here yet" | 2026-07-24 | UX | — |
| **Variants — edit propagation (trigger)** | 🟡 Logic verified | Simulation against the real generator: converges in exactly **1** write; colours-only edit regenerates terms; removed colour stops matching; unrelated write costs **0** writes; 81 terms / 535 bytes for a fully-populated product. Replaced a `TEXT_FIELDS !==` guard that compared arrays by reference | 2026-07-24 | Search | See e2e row |
| **Variants — end-to-end: Firestore leg** | ✅ **Verified** | Real product created through the real write path against production. `indexProductCreate` wrote variant terms in **5s** (37 terms); `black, white, m, xl, cotton` all searchable; a colours-only edit propagated in **0s** with `red` added **and** `black` removed. That last step is also the deployment proof for the trigger fix — under the old `TEXT_FIELDS` guard a colours edit regenerated nothing. Purged from both stores | 2026-07-24 | Search | — |
| **Variants — end-to-end: Algolia leg** | 🟡 Logic verified | Was blocked by the batch-poisoning defect below (record absent after 7min on two runs). Root cause fixed and the backlog drained; the variant leg itself has **not** been re-probed end-to-end since | 2026-07-24 | Search | Re-run the acceptance probe |
| **Search — Algolia batch poisoning** | ✅ **Verified fixed** | One product with a 195KB base64 image produced a 195,884-byte record; Algolia **rejects the whole batch**, and the queue then failed the entire group rather than the offender — 26+ consecutive upserts lost. Shared sanitiser + per-record isolation deployed (`processAlgoliaQueue`). **Production evidence: queue went from `{done:147, dlq:152}` → `{done:306, dlq:0}` after releasing 153 stalled entries — every one drained, zero failures.** Regression: `npm run test:algolia-isolation` 14/14 | 2026-07-24 | Search | — |
| **Variants — Algolia index settings** | ✅ **Verified** | Live `sokoni_products` re-read after push: variant fields searchable **and** facetable, 7 → 13 each. Applied **additively**; the wider live-vs-repo settings drift is recorded as open in [[KNOWN_LIMITATIONS]] | 2026-07-24 | Search | — |
...
**Roll-up (2026-07-24):** **6 ✅** · 5 🟡 · 17 🟠 · 6 ⏳ · 2 🚫 · **1 ❌**
*(The first ❌ in this document: backfill/trigger schema parity. It is recorded
as a failure rather than a gap because it was **demonstrated** — two records for
the same document, 6 fields vs 28. A known failure with a falsifiable fix is
worth more than an unexamined assumption.)*
