...
| **Variants — end-to-end: Algolia leg** | 🚫 **Blocked** (not a variant defect) | Record absent after 7min on two runs. Root cause found: one product with a 195KB base64 image produces a 195,884-byte record and Algolia **rejects the whole batch**; 26+ consecutive queue upserts fail on the same poison record. Pre-existing and unrelated to variants — see [[KNOWN_LIMITATIONS]] | 2026-07-24 | Search | Oversized-record batch poisoning |
