...
- **C2 — `permsVersion` semantics fixed before coding.** `permsVersion` is a **monotonic integer** naming the authority-claim SCHEMA version, defined by a single module constant `CURRENT_PERMS_VERSION` (initial value **1** for this schema). The canonical writer **stamps `CURRENT_PERMS_VERSION` on every mutation** (idempotent): absent → set to current; present and lower → set to current; present and equal → unchanged. It is a schema marker (and a future token-invalidation signal), **not** a per-user counter the writer increments arbitrarily. No other value semantics are introduced during coding.
...
- **[C2] Versioning:** `permsVersion` idempotent — absent → `CURRENT_PERMS_VERSION`; lower → current; equal → unchanged.
