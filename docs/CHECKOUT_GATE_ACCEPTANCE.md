...
- **2026-08-04 — CANONICAL KASS SELLER CONSOLIDATED to D5Ql2 (supersedes the xrH2 Step-2 target).** Pre-flight caught that the products' `sellerUid` had reverted from `xrH2` back to **`D5Ql2`** — a localStorage→Firestore product sync (`_syncedAt` 13:09) re-attributes products to the logged-in operator account (D5Ql2 / alexochieng3030), overwriting the separate `xrH2` shop entity. This is the account-fragmentation / localStorage-sync issue (R1.1) surfacing as a gate blocker: D5Ql2 had no seller profile/deliveryConfig, so a live order would misattribute (#1/#6). **Owner decision: consolidate KASS onto D5Ql2** (work WITH the sync). Applied via `scripts/qa/consolidate-kass-seller.js`: `sellers/D5Ql2` = "KASS SHOP" active + deliveryConfig (distance base100/perKm20/free3000); `users/D5Ql2.roles += seller`. 102 products attribute to D5Ql2. Pre-flight now green (buyable 102/102, deliveryConfig live, rider dispatchable). **The canonical KASS seller for the acceptance record is `D5Ql2`, not `xrH2`.** Caveat: D5Ql2 is also the provisioned rider → for this run seller=rider (solo owner test); buyer MUST be a different account. Durable fix (stop the sync clobber / one canonical account) = R1.1.
