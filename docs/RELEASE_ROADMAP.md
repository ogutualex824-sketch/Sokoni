...
**Governing rule:** nothing on this roadmap starts until **RC Exit is genuinely complete** — the product checkout payment path deployed, Kass Shop `deliveryConfig` applied, one real production order completed, `docs/CHECKOUT_GATE_ACCEPTANCE.md` signed, Release Baseline established. Until then the platform is a Release Candidate under freeze ([[feedback_rc_change_policy]] / `docs/CHECKOUT_GATE_ACCEPTANCE.md`).

> **Payment-path pivot (2026-08-04).** RC exit no longer runs on `darajaSTKPush`. That is a DIRECT-to-seller Daraja flow requiring the seller's own Daraja API credentials in `shopSettings/{sellerUid}` — a collection with **zero documents**, and the KASS owner has a till (3588275) but **no Daraja API access**. Product checkout was re-pointed to the **keyless IntaSend collector** (`initiateSTKPush`, SOKONI Merchant-of-Record using platform keys), with the IntaSend webhook (`webhookIntasend`) now finalising product orders (paid + atomic stock decrement + seller wallet credit) via the shared `_finalizeMarketplacePayment()` helper. The gate's reference transaction runs through THIS path. Live rev: `webhookIntasend-00046`; client commit `e5e1350`.
>
