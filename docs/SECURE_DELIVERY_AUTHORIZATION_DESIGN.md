...
- **VERIFIED 2026-08-07 (evidence trace):**
  - ✅ **Rider-safety core HOLDS.** `deliveryFee` (`safeDeliveryFee`) is stored intact on the session (`index.js:2439`), copied unchanged to the order (`:2700`), and read by both Rail 1 (`:2927`) and seller settlement (`order-settlement.js:40`). **No promo/discount ever mutates it** — the discount reduces `serverTotal` only (`:2424`), and is explicitly capped at goods value (`:2356`, "delivery is still owed"). Rider is always paid the real fee; free delivery never underpays the rider. Seller is settled on `total − delivery` (not over-settled by delivery).
  - ❌ **`deliveryChargedToBuyer` does not exist** — the buyer-vs-rider delivery split is not modelled as separate fields; the buyer's delivery charge is implicit in `serverTotal`.
  - ❌ **Free delivery is a generic discount**, not a delivery-specific funded line (`free_delivery` promo = fixed `maxDiscountCents` on the grand total).
  - ❌ **DEFECT — `fundedBy` is ignored at settlement.** `order-settlement` computes seller gross = `total − delivery = goods − discount` and never reads `fundedBy`, so the **seller silently absorbs every discount**, including platform-funded free delivery. The `fundedBy` funding ledger (`finos.js` `applyPromoCode`) is a separate helper the main checkout (`createCheckoutSession` → `validatePromoCode`) does not call. No delivery-subsidy ledger line; no "Delivery Discount (funder)" receipt line.
- **Genuinely-zero delivery** (KES-0 pickup, or a test order with no rider) is distinct: `deliveryFee = 0` legitimately means no rider payout.
- **Recommended follow-up (own money-path slice, needs sign-off):** honor `fundedBy` at settlement — when `fundedBy='platform'`, add the discount back to the seller's settlement gross (platform absorbs via `PLATFORM_PROMOS`); persist `deliveryFee` vs `deliveryChargedToBuyer` + a subsidy ledger line + receipt line. Does NOT block delivery-auth Phase 0 (which touches verification, not fees).
