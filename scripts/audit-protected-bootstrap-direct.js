...
for (const file of files) {
  ...
  results.push({ file: rel, requiresAuth, requiresRole, hasInit, hasFirebase, hasAuthGuard, hasSharedHeader, initIndex, authIndex, sharedHeaderIndex, firebaseIndex, validInitBeforeAuth, validAuthBeforeHeader, valid, issues, scriptOrder: scripts.map((s, i) => ({ index: i, src: s.src })) });
}
const invalid = results.filter(r => !r.valid);
console.log(JSON.stringify({ total: results.length, validCount: results.filter(r => r.valid).length, invalidCount: invalid.length, invalid }, null, 2));
