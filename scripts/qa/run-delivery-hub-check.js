...
(async () => {
  console.log('\n  SELLER DELIVERY HUB — context check\n');
  console.log('  ── contract (static) ──');
  const cOut = contractChecks();
  cOut.forEach(s => console.log('  ' + (s.ok ? 'PASS' : '** FAIL **') + '  ' + s.step +
    (s.detail && !s.ok ? '   ' + JSON.stringify(s.detail) : '')));
  const cFail = cOut.filter(s => !s.ok).length;
  ...
  try {
    ...
    const r = spawnSync(process.execPath, [
      ...
      '--script', path.join(__dirname, 'delivery-hub-context.mjs'),
      '--timeout', '60000',
    ], {
      encoding: 'utf8',
