...
function makeSurface(uid, opts) {
  ...
  /* addToWishlist calls _homeAgeGuard (f8c5b5a/17eef7a), which calls
  ...
  const body = ['_explicitlyAgeRestricted', '_homeAgeGuard', 'addToWishlist']
    .map((n) => extractFn('script.js', n)).join('\n');
  const vm = require('vm');
