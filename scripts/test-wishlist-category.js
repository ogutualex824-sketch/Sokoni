 ...
 *
 *   firebase emulators:exec --only firestore --project sokoni-wishlist-cat-test \
...
process.env.GCLOUD_PROJECT = process.env.GCLOUD_PROJECT || 'sokoni-wishlist-cat-test';
...
(async () => {
  console.log('\nPHASE 4.2 — CATEGORY WISHLIST WIRING\n' + '='.repeat(58));
  await wipe();
  ...
  ck('C: success toast shown AFTER the write', s.notes.some(n => /Added to wishlist/.test(n.msg) && n.type === 'success'));

  /* D — repeated add does not duplicate */
  console.log('\nD. Repeated add');
  await s.call('catp1'); await s.call('catp1'); await s.call('catp1');
  ck('D: still exactly one canonical record', (await countFor(A)) === 1, await countFor(A));
  ck('D: repeat says "Already in wishlist"', s.notes.some(n => /Already in wishlist/.test(n.msg)));
  ...
  console.log('\nJ. Legacy writer scan of category.js');
  const src = fs.readFileSync(path.resolve(__dirname, '..', 'category.js'), 'utf8');
  ...
  inc('Browser/App Check end-to-end', 'category.html not exercised in a real browser session; DOM shimmed in Node');
