...
User-facing actions return immediately; heavy work (email, webhooks, eTIMS, analytics,
inventory recalc, reports, bulk ops, AI, loyalty, POS sync) runs reliably in the
background with priority queues, exponential-backoff retries, dead-letter queue,
distributed locking, and full admin observability.

### New Cloud Functions (`functions/async-jobs-engine.js` — 19 CFs)
**Core**
- `createJob` — Create any job with idempotency key, priority, scheduled-for, maxRetries
- `getJob` — Fetch status, progress, result, error for caller's own jobs
- `getMyJobs` — List caller's job history (filter by status/type)
- `cancelJob` — Cancel QUEUED / WAITING / RETRYING jobs
- `submitEmailJob` — Convenience: create HIGH-priority EMAIL job
- `submitWebhookJob` — Convenience: create NORMAL-priority WEBHOOK job
**Admin**
- `retryJob` — Reset FAILED / CANCELLED / DEAD_LETTER back to QUEUED
- `replayDLQJob` — Clone DLQ entry as a fresh QUEUED job and remove original
- `pauseQueue` / `resumeQueue` — Halt / restore a named priority queue
- `getQueueDepth` — Counts by status + priority + queue pause states
- `getJobDashboard` — 24h summary, success rate, avg/p95 exec time, byType, byStatus
- `inspectJob` — Full job doc with secrets masked (checks both jobs + DLQ)
- `bulkCancelJobs` — Cancel QUEUED/WAITING by type + priority + age
- `getWorkerStats` — Active workers with heartbeat age
**Scheduled + Trigger**
- `processJobQueue` — every 1 min; claims up to 8 jobs per priority queue
- `onJobCreated` — Firestore trigger; immediately dispatches CRITICAL + HIGH jobs
- `jobStuckRecovery` — every 5 min; reclaims jobs with expired worker locks
- `jobCleanupScheduled` — daily 03:30 EAT; deletes old completed/cancelled jobs

### New Handler Module (`functions/async-job-handlers.js`)
Registry of 28 handlers (not CF exports — internal library):
EMAIL (SendGrid), SMS, PUSH (FCM multicast), RECEIPT, PDF, ETIMS, WEBHOOK (HMAC-SHA256),
ANALYTICS, INVENTORY_RECALC, REPORT, IMAGE_OPT, AI_PROCESS, PRODUCT_INDEX, SEARCH_INDEX,
RECO_UPDATE, LOYALTY_UPDATE (1pt/KES 100), SELLER_NOTIFICATION (Firestore + FCM),
CUSTOMER_SEGMENT, BULK_IMPORT, BULK_EXPORT, SCHEDULED_CLEANUP, DB_MAINTENANCE,
REDIS_WARMUP, EXTERNAL_SYNC, BACKUP_VERIFY, PDF, ANALYTICS_AGGREGATE

### New Files
- `async-jobs.html` — Admin dashboard: KPI strip (8 cards), queue pills, type breakdown,
  priority breakdown, failed jobs table, DLQ table, worker list, queue pause/resume,
  bulk cancel, job inspect, retry/replay tools; auto-refreshes every 30s
- `sokoni-async-jobs.js` — Client SDK: `SokoniAsyncJobs.create()`, `.sendEmail()`,
  `.deliverWebhook()`, `.status()`, `.myJobs()`, `.cancel()`, `.watch()` (Firestore
  realtime), `.waitFor()` (poll to completion), `.admin.*` namespace

### New Firestore Collections
- `asyncJobs` — Active/historical jobs; users read own; admins read all
- `asyncJobsDLQ` — Dead-letter queue; admin only
- `asyncQueues` — Queue pause state per priority; admin only
- `asyncWorkers` — Live worker registry; admin read only
- `posReceipts` — Receipt documents generated by RECEIPT handler

### Modified Files
- `functions/index.js` — 19 new exports wired (async-jobs-engine.js block)
- `firestore.rules` — 5 new collection rules (asyncJobs, asyncJobsDLQ, asyncQueues, asyncWorkers, posReceipts)
- `service-worker.js` — CACHE_VERSION bumped to `sokoni-20260628-async-jobs-v1`; async-jobs.html + sokoni-async-jobs.js added to PRECACHE_STATIC

### Architecture
- **Priority queues**: CRITICAL(0) → HIGH(1) → NORMAL(2) → LOW(3) → BACKGROUND(4)
- **Worker model**: Scheduler polls Firestore every minute; CRITICAL/HIGH get an immediate `onDocumentCreated` trigger in parallel
- **Distributed locking**: Firestore transaction claim + `lockedUntil` TTL (5 min); `jobStuckRecovery` reclaims expired locks
- **Retry**: Exponential backoff `min(base × 2^n, 1h) + jitter(0–1s)`; configurable maxRetries (0–10); `canRetry(err)` per handler
- **Dead Letter Queue**: Jobs moved to `asyncJobsDLQ` after max retries; replayed as new jobs via `replayDLQJob`
- **Idempotency**: SHA-matched `idempotencyKey` per owner; deduplicated at create time
- **Security**: All CFs `enforceAppCheck: true`; secrets masked in `inspectJob`; audit log (20 entries) on every job

### Security
- Secrets masked (password, token, key, card, pin, cvv, pan, apiKey) in `inspectJob` response
- All write paths CF-only in Firestore rules — no client can write job documents
- Admin CFs validate `auth.token.admin || auth.token.superAdmin` before any mutation

### Performance
- Firestore `.count()` queries for queue depth (no document reads)
- CRITICAL/HIGH jobs dispatched within seconds via Firestore trigger (not waiting 60s for scheduler)
- Scheduler batch size: 8 jobs per run × all priority levels = up to 40 concurrent claims per minute
- Cleanup removes completed jobs after 30 days to keep collection lean

---

## [2026-06-28] — Marketplace ↔ POS Integration v1.0

### Summary
Bridges the SOKONI Marketplace and SmartPOS. Inventory is shared via the same
`sellers/{uid}/products` collection. New click-and-collect order flow atomically
reserves stock at purchase and restores it on cancellation. Unified reporting
gives managers one view across POS, marketplace, and C&C channels.

### New Cloud Functions (`functions/pos-marketplace-sync.js` — 7 CFs)
- `createClickAndCollect` — Buyer places pickup order; stock reserved atomically in Firestore transaction
- `getPendingClickAndCollect` — Cashier loads open C&C queue; real-time via onSnapshot
- `updateClickAndCollectStatus` — State machine: pending→ready→collected; cancel restores stock
- `getUnifiedSalesReport` — Manager report: POS + Marketplace + C&C revenue, daily trend, top products
- `syncPromotionToPOS` — Enable/disable a marketplace promotion at the POS register
- `getActivePOSPromotions` — Cashier loads active promotions at checkout
- `getInventoryReserveStatus` — Shows products with stock locked in pending pickup orders

### New HTML Page
- `pos-marketplace.html` — 4-section integration hub: C&C order queue, unified report (Canvas chart), promotion manager, inventory reserve monitor; 1,654 lines

### Modified Files
- `functions/index.js` — 7 new exports wired
- `firestore.rules` — `sellers/{uid}/clickAndCollect` subcollection: buyers can create, sellers can update, no delete
- `service-worker.js` — `/pos-marketplace.html` added to PRECACHE_STATIC

### Architecture Decision
C&C orders stored as `sellers/{uid}/clickAndCollect/{orderId}` subcollection (not top-level) to avoid composite Firestore indexes — project is at the 200-index limit.

### Security
- Stock reservation and cancellation-restoration both run in Firestore transactions (no race conditions)
- Role-gated: cashier(0) for C&C queue; manager(2) for reporting, promotions, inventory reserve
- `esc()` XSS escaper on all user content in pos-marketplace.html
...
### Summary
Transforms SmartPOS from feature-complete into world-class. Focus on UX excellence, merchant onboarding,
daily operational workflows, live observability, and market readiness. No new backend modules —
pure polish, speed, and reliability across all existing surfaces.

### New HTML Pages
- `pos-onboard.html` — 5-step first-time merchant wizard (Business Setup → Products → Hardware → Payment → Demo Sale); CSV drag-and-drop import; STK push test; confetti on completion; localStorage progress persistence; 2,107 lines
- `pos-daily.html` — Daily Operations Hub (Morning/Trading/Closing sections auto-expanded by time of day); 11 CF calls; opening cash count; inventory alerts; live metrics strip; approval countdowns; cash reconciliation variance bar; confetti on day close; 1,950 lines
- `pos-observability.html` — Live Operations Center; 6 sections: System Health tiles, Active Sessions (onSnapshot), Payment Health funnel, Hardware Status Grid, Alert Center, Sync & Integration Status; 15-second auto-refresh; `Promise.allSettled` resilience; 2,060 lines

### Modified Files
- `pos.html` — 10 surgical UX improvements: (1) bottom quick-nav strip for mobile, (2) empty cart state with MutationObserver, (3) charge button spinner + double-tap guard, (4) payment method visual clarity (M-Pesa green/Cash green/Card blue), (5) customer tier badges as colored pills, (6) product search auto-focus + Enter-to-add, (7) 44×44px quantity tap targets + editable number inputs, (8) `showProfile()` replaced `alert()` with modal, (9) keyboard shortcuts (`/` search, `?` help, `Escape` clear, `Enter` pay), (10) iOS 16px font fix; +583 lines
- `service-worker.js` — Cache version `sokoni-20260628-smartpos40-v1`; added `pos-onboard.html`, `pos-daily.html`, `pos-observability.html` to PRECACHE_STATIC

### New Documentation
- `SMARTPOS_ENTERPRISE_LAUNCH_REPORT.md` — Full enterprise launch readiness report with performance benchmarks, hardware compatibility matrix, security audit, real-world merchant testing checklists (6 business types), deployment checklist (P0/P1/P2), production readiness score

### Deployment Blocker (Action Required)
SmartPOS 3.0's 139 new CFs are code-complete but blocked by Cloud Run service quota (project at 1,017/1,000 services in us-central1). To resolve:
1. Google Cloud Console → IAM & Admin → Quotas → Cloud Run Admin API → Cloud Run services (us-central1)
2. Request limit increase to 1,300
3. Once approved: `firebase deploy --only functions`

### Performance Improvements
- POS checkout screen: 10 UX wins targeting sub-10-second new sale
- Mobile: all inputs 16px (iOS no-zoom), 44×44px tap targets, `env(safe-area-inset-bottom)` safe area
- No more blocking `alert()` / `confirm()` on any POS page
- Empty states and loading shimmers on every async operation
