...
module.exports = {
  ...
  steps: [
    { name: 'Search page loads and accepts a query', capability: 'Search query', async run(ctx) {
    ...
    { name: 'Input font ≥16px (no iOS zoom on the search field)', capability: 'Search input ergonomics', async run(ctx) {
        const page = await ctx.ui();
    ...
    { name: 'Warm-cache path present (localStorage)', capability: 'Search cache', async run(ctx) {
        const page = await ctx.ui();
    ...
    { name: 'Typo + bilingual (kiatu/viatu) resolve', capability: 'Search synonyms', async run() {
        throw new BlockedError('synonym/typo resolution needs seeded catalog on a data backend');
    }},
    { name: 'Deleted product disappears from results', capability: 'Search invalidation', async run(ctx) {
